...
public class StringUtils {
    ...
    public static String getyyyyMM(int year, int month){
        String yyyyMMdd= String.format("%04d%02d",year,month);
        return yyyyMMdd;
    }
    public static String getStandardData(int year, int month, int day){
        String yyyyMMdd= String.format("%04d-%02d-%02d",year,month,day);
        return yyyyMMdd;
    }

//    public static String getSalaryByDivision(int num){//金额分段  从后到欠 三位一段
//        String strNum=num+"";
...
//    }
    ...
    public static double stringTodouble(String string) {
        string = string.trim();
        if (!StringUtils.isEmpty(string)) {
            double num = Double.valueOf(string).doubleValue();
            return num;
        } else {
            return 0;
        }
     ...
     * 判断工资类型
     * */
    public static String getSalaryType(int type){
        String strSalary="";
        switch (type) {
            case 1:
                strSalary = "元/周";
