...
public class UpdateVersionUtil {
    ...
    public UpdateVersionUtil(Context context) {
        ...
        if (AppUtils.getAppInfo(mContext)!=null&&AppUtils.getAppInfo(mContext).getName()!=null){
            apkName=AppUtils.getAppInfo(mContext).getName()+".apk";
        }
    }
    ...
    public void downloadBackground(String downloadUrl) {
        ...
        String path = Environment.getExternalStorageDirectory()
                + ConfigConstant.DOWNLOAD_PATH + apkName;
        ...
        if(SDCardUtils.getFreeSpace(ConstUtils.MemoryUnit.MB)>fileSize){
            ...
            // 外部存储
            request.setDestinationInExternalPublicDir(ConfigConstant.DOWNLOAD_PATH,
                    apkName);
    ...
    public void installApk(String path, Context context) {
        ...
        if (Build.VERSION.SDK_INT>= Build.VERSION_CODES.N){
            Uri contentUri = FileProvider.getUriForFile(context, "com.rkl.fileProvider", apkfile);
