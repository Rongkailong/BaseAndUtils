package com.rkl.common_library.expandlist;
...
public class NestFullViewHolder {
     ...
     * @return
     */
    public NestFullViewHolder setImageByUrl(int viewId, String url) {
        GlideLoader.getInstance().displayImageSkipCache(mContext, url, (ImageView) getView(viewId), 0, 0);
        return this;
    }
    /**
     *  为ImageView设置网络图片(有缓存)
     * @param context
     * @param viewId
     * @param defaultImage
     * @param url
     * @return
     */
    public NestFullViewHolder setImageByUrl(Context context, int viewId, int defaultImage, String url) {
        GlideLoader.getInstance().displayImage(context,url,(ImageView) getView(viewId), defaultImage,defaultImage);
        return this;
    }
    /**
     * 为ImageView设置圆形网络图片(用于加载用户头像)
     *
     * @param viewId
     * @return
     */
//    public NestFullViewHolder setCircleImgByUrl(Context context, int viewId, String url, boolean isCache) {
//        GlideLoader.getInstance().displayCircleTransfrom(context,url, (ImageView) getView(viewId),isCache);
//        return this;
//    }
    ...
    public NestFullViewHolder setVisible(int viewId, int visible) {
        View view = getView(viewId);
